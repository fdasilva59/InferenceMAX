...
PY
  ...
  python3 -m lm_eval --model local-completions \
    ...
    --model_args "model=$MODEL,base_url=$OPENAI_COMP_BASE,api_key=$OPENAI_API_KEY,max_retries=1,num_concurrent=1,timeout=300,tokenized_requests=False" \
    --gen_kwargs "max_tokens=2048,temperature=0,top_p=1"
    RC=$?
  set +x
